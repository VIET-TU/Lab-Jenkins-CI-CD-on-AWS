pipeline {
    agent any
    environment{
        FULL_IMAGE = "287925497349.dkr.ecr.ap-southeast-1.amazonaws.com/nodejs-random-color:${VERSION}"
        TASK_DEFINITION =""
        NEW_TASK_DEFINITION=""
        NEW_TASK_INFO=""
        NEW_REVISION=""
    ...
    stages {
        stage('Checkout using tag') {
            steps{
                checkout([
                    ...
                    extensions: [], 
                    submoduleCfg: [], 
                    userRemoteConfigs: [[url: 'git@github.com:VIET-TU/nodejs-random-color.git', credentialsId: 'github-jenkins-private-key']]
                ])
            }
        ...
        stage('Upload image to ECR') {
            ...
            steps {
                ...
                sh 'docker tag nodejs-random-color:${VERSION} 287925497349.dkr.ecr.ap-southeast-1.amazonaws.com/nodejs-random-color:${VERSION}'
                
                sh 'docker push 287925497349.dkr.ecr.ap-southeast-1.amazonaws.com/nodejs-random-color:${VERSION}'
            }
        }
